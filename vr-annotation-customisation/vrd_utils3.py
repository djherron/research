...
# -*- coding: utf-8 -*-
"""
Created on Fri Jun 25 16:01:56 2021

@author: dave
"""

'''
This module contains utility functions that support the implementation of 
specific types of customisations to the visual relationships contained in
the VRD image annotations. 

A VRD visual relationship annotation can be described informally as a 
triple consisting of (subject, predicate, object). For example, we might
have (person, ride, horse).

Each visual relationship within the set of visual relationships 
representing the annotations for a particular VRD image is, in memory,
represented by a Python dictionary with the following format:
...
values into the list of names maintained in the file `objects.json`. They
thus map directly to the corresponding object name (text label). For
example, 0 maps to 'person' and 43 maps to 'horse'. Similarly, the 
'predicate' category (35) is an integer index into the list of names
maintained in file `predicates.json`. It thus maps directly to the
corresponding predicate name (text label) (e.g. 'ride').

The bounding boxes for the detected objects (the 'subject' of the
relationship and the 'object' of the relationship) have the format
[ymin, ymax, xmin, xmax] in the VRD image annotations.

The annotations for an image is, in memory, a Python list of dictionaries
of the form specified above. A specific relationship can thus be accessed by
indexing into that list.  Our approach to specifying and implementing
visual relationship customisations relies heavily on that fact.

The functions defined in this module support the customisation of the
list of dictionaries that represents the annotations for a VRD image.
'''
...
import json
import sys
sys.path.insert(0, '../annotationAnalysis')
