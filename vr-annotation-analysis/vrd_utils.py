...
VRD images and their visual relationship annotations.

The utility functions in this module perform tasks that fall into the
following categories:
* load data
* get image attributes
* display images
* get image annotation content
* find images whose annotations satisfy some criterion

No function in this module updates VRD annotations in any way.

The functions in this module can safely be used against the VRD
images in combination with either the original VRD visual relationship
annotations or the customised VRD visual relationship annotations.  
'''
...
def test1_calc_bbox_pair_iou():
    b1 = [1,4,1,4] 
    b2 = [2,5,2,5]
    iou = calc_bbox_pair_iou([b1,b2])
    assert iou == 4.0 / 14.0
    return None

def test2_calc_bbox_pair_iou():
    b1 = [379, 396, 370, 393] 
    b2 = [380, 400, 369, 397]
    iou = calc_bbox_pair_iou([b1,b2])
    print(f'iou: {iou}')
